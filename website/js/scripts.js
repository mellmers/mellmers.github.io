var zoomLevel = 1;
var routenplanerForm = "<form id='routenplaner-form'>" +
    "<div class='input-group'>" +
    "<input type='text' class='form-control input-lg' placeholder='Route zur Tischlerei berechnen (Geben Sie hier Ihre Adresse ein)'>" +
    ...
    $('#contactform').submit(function(){
        ...
        $.post(
            ...
            function(data){
                ...
                var $alertWrapper = $("#contact-alert-wrapper");
                if(data.success && data.error == null) {
                    $alertWrapper.html(
                        "<div class='alert alert-success alert-dismissible' role='alert'>" +
                            "<button type='button' class='close' data-dismiss='alert' aria-label='Close' title='Schließen'><span aria-hidden='true'>&times;</span></button>" +
                            "<span>Deine Nachricht wurde erfolgreich an uns verschickt.</span>" +
                        "</div>"
                    );
                } else {
                    $alertWrapper.html(
...
(function($, window, document, undefined) {
    ...
    var $inputs = $("#contactform .contacting input");
